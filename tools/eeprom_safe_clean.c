...
static DWORD do_ctrl(HMODULE wdapi, HANDLE hDev,
    ...
    fflush(stdout);
    /* WDU_Transfer: hDevice, pipe, fRead, options, buf, len, pTransferred, setup, timeout */
    DWORD res = fn(hDev, 0, is_read, 0, buf?buf:dummy, len?len:1, &tr, setup, 3000);
    return res;
}
...
int main(void) {
    ...
    words[1] = nv; words[2] = np;
    WORD chk = 0xAAAA;
    for (int i=0; i<nw-1; i++) { chk^=words[i]; chk=(WORD)((chk<<1)|(chk>>15)); }
    words[nw-1] = chk;
    ...
    printf("  VID: 0x%lX  PID: 0x%lX  CHK: 0x%lX\n", w1, w2, w3);
    fflush(stdout);
    ...
    /* Verify */
    BYTE vb[2] = {0};
    do_ctrl(wdapi, g_hDev, 0xC0, 0x90, 0, 1, vb, 2, NULL);
    WORD vv = (WORD)(vb[0]|(vb[1]<<8));
    do_ctrl(wdapi, g_hDev, 0xC0, 0x90, 0, 2, vb, 2, NULL);
    WORD vp = (WORD)(vb[0]|(vb[1]<<8));
    printf("Verify: VID=0x%04X PID=0x%04X\n", vv, vp);

    if (vv==nv && vp==np)
        printf("\nSUCCESS! Reconnect to get FTDI COM port.\n");
    else
        printf("\nWARNING: Verify mismatch!\n");
